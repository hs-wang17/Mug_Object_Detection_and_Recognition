...
class MyTestDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            img = self.transform(img)
        img_origin = self.data[index]
        return img, label, img_origin
...
if EVAL == True:
    epoch = 255
    model = MyModel(num_classes=total_class * 5)
    model.load_state_dict(torch.load('./classifier/model_CNN_single/epoch-%d.pt' % epoch))
    model.to(device)
    IoU_total = 0
    count = 0
    for inputs, labels, img in test_loader:
        inputs, labels = inputs.to(device), labels.to(device)
        batchsize_local = labels.shape[0]
        outputs = model(inputs)
        output_classification = outputs[:, :total_class * 1]
        output_regression = outputs[:, total_class * 1:]
        ...
        for i in range(batchsize_local):
            ...
            image = image.resize((1137, 640))
            fig, ax = plt.subplots(1)
            ax.imshow(image)
            box = output_regression[i].cpu().reshape(total_class,4)
            box_real = labels_regression[i].cpu().reshape(total_class,4)
            for j in [1, 4, 6]:
...
for epoch in trange(num_epochs):
    ...
    if not epoch % 10:
        torch.save(model.state_dict(), "./classifier/model_CNN_single/epoch-%d.pt" % epoch)
    ...
    with torch.no_grad():
        ...
        if epoch_loss < min_losss_val:
            ...
            torch.save(model.state_dict(), "./classifier/model_CNN_single/epoch-6344.pt")
